...
2. Run the script: `.\windows_cleaner.ps1`
...
* **DISM RestoreHealth**: This is the slowest step and can take **10–30 minutes** with no visible progress (output is suppressed). The process is working as long as `DISM.exe` and `Dismhost.exe` appear in Task Manager — low CPU and RAM usage is completely normal. Do not cancel it. If you don't need system repair, comment out the relevant lines:
  ```powershell
...
* **ResetBase**: The `/ResetBase` flag is intentionally commented out. It permanently removes all Update rollback points — only uncomment if you know what you're doing.
...
* **`/ResetBase` commented out by default** — was previously active, now opt-in with explicit warning.
